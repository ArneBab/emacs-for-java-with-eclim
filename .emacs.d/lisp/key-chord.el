...
;; Copyright (C) 2015 Arne Babenhauserheide
;;
...
;; ########   Compatibility   ########################################
;;
;; Works with Emacs-20.3, 20.6, 20.7, 21.2, 21.4, 22.1, 23.4, 24.5.
...
;; To make it less likely that chords are fired during regular
;; writing, key-chord-mode tracks the last few delays between
;; keys. Chords only fire if their delay is at most the last delay or
;; an estimate from key-chord-averaging-window-length keystrokes
;; (default: 10) (whatever is larger), divided by
;; key-chord-max-delay-divisor (default: 4). Including the last delay
;; ensures that chords always work after a short break.
...
(defvar key-chord-two-keys-delay 0.1	; 0.05 or 0.1
  "Max time delay between two key press to be considered a key
  chord. This is used as lower limit for the estimation of normal
  typing speed.")
...
there are no delay values yet."
  ...
  (if (> 2 (length key-chord-last-delays))
      ...
      (let ((last-delay (car key-chord-last-delays))
        ...
        (/ (max lower-quartile-delay last-delay)
